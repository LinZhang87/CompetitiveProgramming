...
public class CycleDetection {
    ...
    /*
        undirected graph, so we need to make sure that we only process one of the the same edge (u, v) and (v, u)
        if we assume the index i is the node label, then we can ensure that we only process edges whose other end
        //node is >= the current node's label
     */
